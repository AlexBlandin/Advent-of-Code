...
test "examples" {
    try expect((try day1("abc", test_alloc)).p1 != 0);
    try expect((try day1("abc", test_alloc)).p2 != 0);
}

test "answers" {
    try expect((try day1(input, test_alloc)).p1 != 0);
    try expect((try day1(input, test_alloc)).p2 != 0);
}
