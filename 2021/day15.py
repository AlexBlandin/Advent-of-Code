...
path = list(map(list, map(partial(map, int), Path("data/day15.txt").read_text().splitlines())))
path = [[1, 1, 6, 3, 7, 5, 1, 7, 4, 2], [1, 3, 8, 1, 3, 7, 3, 6, 7, 2], [2, 1, 3, 6, 5, 1, 1, 3, 2, 8],
...
def neighbours(x: int, y: int, visited: set[tuple[int, int]] = visited):
  return [(px, py) for px, py in [(x + 1, y), (x, y + 1), (x - 1, y), (x, y - 1)]
          if legal(px, py) and (px, py) not in visited]
...
def depth(
  x: int,
  y: int,
  path: list[list[int]] = path,
  visited: set[tuple[int, int]] = visited,
  target: tuple[int, int] = (width - 1, height - 1)
):
  if (x, y) in visited: return inf
  visited.add((x, y))
