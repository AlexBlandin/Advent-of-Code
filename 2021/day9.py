  ...
  if xy not in a_basin:
    ...
    while len(search):
      ...
      search |= {
        ...
        if 0 <= a < side and 0 <= b < side and heightmap[a][b] != 9 and (a, b) not in a_basin
      }
      for ab in [
        (a, b) for a, b in [(x + 1, y), (x, y + 1), (x - 1, y), (x, y - 1)] if 0 <= a < side and 0 <= b < side and heightmap[a][b] != 9 and (a, b) in a_basin
      ]:
        if a_basin[ab] != xy:
