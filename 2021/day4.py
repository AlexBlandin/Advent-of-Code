...
lines = lines[2:]
boards = [[list(map(int, r.strip().split())) for r in b] for b in zip(lines[0::6],lines[1::6],lines[2::6],lines[3::6],lines[4::6])]
...
for k, bk in order.items():
  ...
  for i in bk:
    ...
    five_in_a = lambda row: len(set(row)&seen)==5
    if i not in won and (any(filter(five_in_a, boards[i])) or any(filter(five_in_a, zip(*boards[i])))):
