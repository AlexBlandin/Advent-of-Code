...
lengths = [[*Path("day14.txt").read_bytes().strip(), ord("-"), *list(f"{i}".encode()), 17, 31, 73, 47, 23] for i in range(128)]
...
def group():
  ...
  for x, y in ((x, y) for x in range(128) for y in range(128)):
    if srow[y][x]:
      nbs = [(x, y) for x, y in ((x, y), (x, y - 1), (x - 1, y), (x, y + 1), (x + 1, y)) if -1 < x < 128 and -1 < y < 128 and srow[y][x]]
