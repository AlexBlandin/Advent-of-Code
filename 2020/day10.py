...
# from functools import lru_cache
# _,__,b,G,r = (a:=[0]+sorted([int(l.strip()) for l in open("data/day10.txt").readlines()])),(a:=a + [a[-1]+3]),[y-x for x,y in zip(a,a[1:])], {x:[y for y in range(x+1,x+4) if y in a] for x in a},lru_cache(lambda x: 1 if x==a[-1] else sum(r(y) for y in G[x]))
