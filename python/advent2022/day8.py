...
forest = [
  [peaks(row) for row in trees] for trees in [trees, backwards(trees), transpose(trees), backwards(transpose(trees))]
]
sightlines = [
  (vis, (W - x - 1, x, H - y - 1, y))
  for y, rows in enumerate(
    zip(forest[0], backwards(forest[1]), transpose(forest[2]), transpose(backwards(forest[3])), strict=True)
  )
